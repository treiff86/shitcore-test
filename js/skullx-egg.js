...
<head>
    ...
    <link rel="stylesheet" href="bonusstage.css?v=9">
</head>
<body class="overflow-x-hidden min-h-screen flex flex-col justify-between">
            ...
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <div class="flex items-center space-x-2">
                        ...
                        <button id="conmenEggTestBtn" onclick="testThemeEasterEgg()" title="Master wallet only: force-trigger whichever theme's easter egg matches the currently active theme" class="hidden items-center gap-1 text-[10px] bg-gradient-to-b from-gray-400 to-gray-600 hover:from-gray-300 hover:to-gray-500 text-black px-2 py-0.5 rounded font-bold normal-case shadow">
                    ...
                    <div>
                        ...
                        <div class="relative w-full h-[300px] bg-[#040609] rounded-lg border border-[#1A2232] overflow-hidden">
                            ...
                            <div id="mcChartPainterWrap" class="hidden mc-chart-painter absolute bottom-0 right-1 w-28 h-28 md:w-36 md:h-36 pointer-events-none select-none">
                                <img src="assets/Reiffer painting.webp" alt="" class="mc-painter-body absolute inset-0 w-full h-full">
                                <img id="mcChartPainterTorso" src="assets/Reiffer painting.webp" alt="" class="mc-painter-torso absolute inset-0 w-full h-full">
                                <img id="mcChartPainterArm" src="assets/Reiffer painting.webp" alt="" class="mc-painter-arm absolute inset-0 w-full h-full">
                            </div>
                        </div>
                    </div>
            ...
            <div class="relative">
                <div id="onlineLobbySoonOverlay" class="absolute inset-0 z-20 bg-black/70 backdrop-blur-sm rounded-xl flex flex-col items-center justify-center gap-4 pointer-events-auto">
                    ...
                    <div class="text-center text-[11px] md:text-xs text-gray-300 space-y-1">
                        ...
                        <div class="text-gray-500"><span class="uppercase tracking-wide">Coming soon:</span> Bitcoin Wizards &bull; SkullX &bull; Genuine Undead &bull; $MIM SOL/BTC Holders</div>
                    </div>
                </div>
                <div class="space-y-6">
    ...
    <div id="toastDefault" class="fixed bottom-6 right-6 z-50 bg-[#0C0F16] border-l-4 border-blue-500 px-5 py-3.5 rounded-lg shadow-2xl flex items-center space-x-3 text-sm text-gray-100 hidden max-w-sm">
        <img id="toastImageDefault" src="" alt="" class="hidden w-12 h-12 rounded object-cover flex-shrink-0 border border-white/10">
    ...
    <div id="toastMedieval" class="fixed bottom-0 left-6 z-50 hidden flex items-end gap-2 max-w-md">
        <img src="assets/pfp.webp?v=2" alt="" class="w-48 h-auto flex-shrink-0 drop-shadow-2xl">
    ...
    <div id="toastConmen" class="fixed bottom-0 left-6 z-50 hidden flex items-end gap-2 max-w-md">
        <img src="assets/conmen-pfp.webp?v=2" alt="" class="w-48 h-auto flex-shrink-0 drop-shadow-2xl">
    ...
    <div id="mcdEggPopup" class="fixed bottom-0 left-6 z-[60] hidden flex items-end gap-2 max-w-md" style="pointer-events: auto;">
        <img src="assets/mcdonalds-pfp.webp" alt="" class="w-40 h-auto flex-shrink-0 drop-shadow-2xl">
    ...
    <!-- Wallet picker - now the one and only wallet-connect entry point,
         covering both chains in clearly separated sections instead of
         the old setup (auto-connect if exactly one Solana wallet was
         found, a completely separate TEST-only modal for Bitcoin). -->
    <div id="walletPickerModal" class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 hidden p-4">
        <div class="bg-[#0C0F16] border border-cyan-500/40 max-w-sm w-full p-6 rounded-xl shadow-2xl space-y-3">
            ...
            <p class="text-[10px] text-gray-500">More than one wallet was found on this browser - pick which one to connect with.</p>

            <div>
                <div class="text-[10px] uppercase tracking-wide text-gray-500 font-bold mb-1.5">Solana</div>
                <div id="walletPickerButtons" class="space-y-2"></div>
            </div>

            <div>
                <div class="text-[10px] uppercase tracking-wide text-orange-500/80 font-bold mb-1.5">Bitcoin</div>
                <p class="text-[10px] text-gray-500 mb-1.5">Unlocks Ordinals-gated content (Skull X, Bitcoin Wizards). Read-only, never asks you to sign a transaction.</p>
                <div class="space-y-2">
                    <button onclick="closeWalletPicker(); connectXverse();" class="w-full py-2.5 bg-orange-600/20 hover:bg-orange-600/40 border border-orange-500/30 text-orange-300 rounded font-bold text-sm transition">Xverse</button>
                    <button onclick="closeWalletPicker(); connectUnisat();" class="w-full py-2.5 bg-orange-600/20 hover:bg-orange-600/40 border border-orange-500/30 text-orange-300 rounded font-bold text-sm transition">UniSat</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="debugMenuModal" class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 hidden p-4">
        <div class="bg-[#0C0F16] border border-amber-500/40 max-w-lg w-full p-6 rounded-xl shadow-2xl space-y-3 max-h-[85vh] overflow-y-auto">
            ...
            <div>
                <div class="flex items-center justify-between mb-1">
                    <div class="text-[10px] uppercase tracking-wide text-gray-500">Fight Sync (Online Fight Club)</div>
                    <button onclick="checkFightSyncStatus()" class="text-[10px] text-amber-400 hover:text-amber-300 transition"><i class="fa-solid fa-satellite-dish mr-1"></i>Check</button>
                </div>
                <div id="fightSyncCheckResult" class="bg-black/40 border border-[#1A2232] rounded p-3 text-[11px] font-mono text-gray-300">Not in an online match right now - start or join a Fight Club room, then check.</div>
            </div>
        ...
        <div class="win95-window">
            <div class="win95-titlebar">
                ...
                <div class="win95-controls">
                    <button class="win95-btn mg-music-btn" onclick="if (typeof toggleMiniGameMusic === 'function') toggleMiniGameMusic()" title="Music: ON (click to mute)"><i class="fa-solid fa-music"></i></button>
                    <button class="win95-btn mg-sfx-btn" onclick="if (typeof toggleMiniGameSfx === 'function') toggleMiniGameSfx()" title="SFX: ON (click to mute)"><i class="fa-solid fa-volume-high"></i></button>
                    <button class="win95-btn" disabled title="Minimize (not implemented)">_</button>
                    <button class="win95-btn" disabled title="Maximize (not implemented)"><i class="fa-regular fa-square"></i></button>
    ...
    <div id="conmenEggPopup" onclick="openConmenLauncher()">
        ...
        <img src="assets/conmen-easteregg.webp" alt="">
    </div>
        ...
        <div class="win95-window-full">
            ...
            <div class="win95-content-full">
                <div class="conmen-launcher-inner">
                    ...
                    <a class="conmen-launcher-btn" href="https://hl2.slqnt.dev/" target="_blank" rel="noopener noreferrer">
    ...
    <div id="skullxEggPopup" onclick="openSkullXLauncher()">
        <span class="skullx-egg-tag">psst...</span>
        <img src="assets/skullx-easteregg.webp" alt="">
    </div>

    <!-- Skull X easter egg: same full-screen launcher pattern as Conmen,
         pointing at the Skull X Linktree instead of a marketplace listing. -->
    <div id="skullxLauncherOverlay" class="fixed inset-0 bg-black hidden z-[110]">
        <div class="win95-window-full">
            <div class="win95-titlebar skullx-titlebar">
                <span class="win95-title"><i class="fa-solid fa-skull"></i> SKULLX_EASTER.EGG</span>
                <div class="win95-controls">
                    <button class="win95-btn" disabled title="Minimize (not implemented)">_</button>
                    <button class="win95-btn" disabled title="Maximize (not implemented)"><i class="fa-regular fa-square"></i></button>
                    <button class="win95-btn win95-close" onclick="closeSkullXLauncher()" title="Close">&times;</button>
                </div>
            </div>
            <div class="win95-menubar">
                <span>File</span><span>Edit</span><span>View</span><span>Help</span>
            </div>
            <div class="win95-content-full skullx-launcher-bg">
                <div class="conmen-launcher-inner">
                    <img src="assets/skullx-easteregg.webp" alt="">
                    <h2>You Found Skull X!</h2>
                    <p>You dug deep enough to find this. Real ones get rewarded - here's where the rest of the collection lives.</p>
                    <a class="conmen-launcher-btn skullx-launcher-btn" href="https://joeheyming.github.io/doom/" target="_blank" rel="noopener noreferrer">
                        <i class="fa-solid fa-arrow-up-right-from-square"></i> Enter Skull X
                    </a>
                </div>
            </div>
            <div class="win95-statusbar">
                <span class="win95-statusfield">Ready</span>
                <span class="win95-statusfield">Skull X Exclusive</span>
            </div>
        </div>
    </div>
...
<script src="js/audio.js?v=19" defer></script>
<script src="js/state.js?v=15" defer></script>
<script src="js/ui.js?v=6" defer></script>
<script src="js/markets.js?v=19" defer></script>
...
<script src="js/perks.js?v=3" defer></script>
<script src="js/bonusstage.js?v=22" defer></script>
<script src="js/fightgame.js?v=43" defer></script>
<script src="js/conmen-egg.js?v=6" defer></script>
<script src="js/skullx-egg.js?v=2" defer></script>
<script src="js/mcdonalds-egg.js?v=4" defer></script>
<script src="js/web3.js?v=49" defer></script>
<script src="js/wizardpopup.js?v=2" defer></script>
<script src="js/win95desktop.js?v=5" defer></script>
<script src="js/btcwallet.js?v=4" defer></script>
<script src="js/onlinesync.js?v=1" defer></script>
<script src="js/onlinelobby.js?v=6" defer></script>
